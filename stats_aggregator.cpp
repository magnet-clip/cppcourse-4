...
template <typename T> ostream &operator<<(ostream &os, const optional<T> &v) {
  if (v) {
    os << *v;
  } else {
    os << "undefined";
...
void Sum::Process(int value) { sum += value; }

void Sum::PrintValue(std::ostream &out) const { out << "Sum is " << sum; }
