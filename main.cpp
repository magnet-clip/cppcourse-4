...
Node *FindDownMinElement(Node *me) {
  while (me->left != nullptr)
    me = me->left;
  return me;
}

Node *FindUpLeftChild(Node *me) {
  if (me->parent == nullptr)
    return nullptr;
  if (me->parent->left == me)
    return me->parent;
  return FindUpLeftChild(me->parent);
}

Node *Next(Node *me) {
  if (me->right != nullptr) {
    return FindDownMinElement(me->right);
  } else {
    Node *left = FindUpLeftChild(me);
    if (left != nullptr) {
      return left;
    } else
      return nullptr;
